...
def analyze_model(
    ...
    # Verify alignment
    assert len(demo_test) == len(X_test), f"Demo ({len(demo_test)}) != X_test ({len(X_test)})"
    assert (demo_test.index == X_test.index).all(), "Index mismatch!"
    
    # Human-readable names
    sex_names = {1: "Male", 2: "Female"}
    education_names = {
        0: "Other", 1: "Graduate School", 2: "University",
    ...
    for name, df in [("SEX", sex_df), ("EDUCATION", edu_df), ("MARRIAGE", mar_df)]:
        if len(df) > 1:
            ...
            print(f"{name}:")
            print(f"  Recall range: {recall_range:.3f} {'⚠️ HIGH' if recall_range > 0.1 else '✓ OK'}")
            print(f"  FPR range:    {fpr_range:.3f} {'⚠️ HIGH' if fpr_range > 0.1 else '✓ OK'}")
            print(f"  FNR range:    {fnr_range:.3f} {'⚠️ HIGH' if fnr_range > 0.1 else '✓ OK'}")
...
def plot_fairness_results(results: dict[str, pd.DataFrame], model_name: str, output_dir: Path) -> None:
    """
    ...
    for ax, (demo_name, df) in zip(axes, results.items()):
        ...
        for i, row in df.iterrows():
            ax.text(i, row["Approval_Rate"] + 0.02, f"{row['Approval_Rate']:.1%}",
                    ha='center', va='bottom', fontsize=9)
    plt.tight_layout()
    plt.savefig(output_dir / f"{model_name}_approval_rates.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    for ax, (demo_name, df) in zip(axes, results.items()):
        ...
        ax.set_ylim(0, 1)
        for i, row in df.iterrows():
            ax.text(i, row["Recall"] + 0.02, f"{row['Recall']:.1%}",
                    ha='center', va='bottom', fontsize=9)
    plt.tight_layout()
    plt.savefig(output_dir / f"{model_name}_recall_by_group.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    for ax, (demo_name, df) in zip(axes, results.items()):
        ...
        ax.set_ylim(0, 1)
        for i, row in df.iterrows():
            ax.text(i, row["FNR"] + 0.02, f"{row['FNR']:.1%}",
                    ha='center', va='bottom', fontsize=9)
    plt.tight_layout()
    plt.savefig(output_dir / f"{model_name}_false_negative_rates.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    for model_file in model_files:
        ...
        try:
            results = analyze_model(model_path, "data/processed/44features.pkl", output_dir)
