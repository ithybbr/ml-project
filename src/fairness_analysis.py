...
Evaluates credit scoring model performance across demographic subgroups
(SEX, EDUCATION, MARRIAGE) to detect potential algorithmic bias.
Calculates detailed metrics including False Positive Rate (FPR),
False Negative Rate (FNR), and Approval Rates per demographic.
"""
...
def load_demographics(split: str = "test") -> pd.DataFrame:
    """
    Loads raw demographic data for a specific dataset split from the serialized pickle file.

    The demographic.pkl file is expected to contain:
    [X_train_demo, X_val_demo, X_test_demo, y_train, y_val, y_test]

    Args:
        split (str, optional): The dataset split to retrieve ("train", "val", or "test").
            Defaults to "test".

    Raises:
        FileNotFoundError: If the demographic.pkl file does not exist.

    Returns:
        pd.DataFrame: A dataframe containing the demographic columns (e.g., X2, X3, X4).
    """
...
def subgroup_metrics(
    ...
    """
    Computes confusion matrix-based fairness metrics for a specific demographic feature.

    Args:
        y_true (pd.Series): The ground truth binary labels.
        y_pred (np.ndarray): The binary predictions generated by the model.
        demo_df (pd.DataFrame): The dataframe containing the demographic features.
        group_col (str): The column name representing the demographic trait to analyze (e.g., "X2").
        group_names (Optional[dict[int, str]], optional): A dictionary mapping numeric group
            codes to human-readable strings (e.g., {1: "Male"}). Defaults to None.

    Returns:
        pd.DataFrame: A dataframe where each row represents a subgroup and columns represent
            calculated performance metrics (Accuracy, FPR, FNR, etc.).
    """
...
def analyze_model(
    ...
    """
    Runs a complete fairness analysis for a single trained model on the test set.

    Args:
        model_path (str | Path): The path to the serialized (.pkl) trained model.
        features_path (str | Path): The path to the processed features dataset used for testing.
        output_dir (str | Path, optional): Directory to save the resulting CSVs. Defaults to "results/fairness".

    Returns:
        dict[str, pd.DataFrame]: A dictionary containing the fairness metrics DataFrames keyed
            by demographic category ("sex", "education", "marriage").
    """
...
def plot_fairness_results(
    ...
    """
    Creates and saves bar charts visualizing the fairness metrics across subgroups.

    Args:
        results (dict[str, pd.DataFrame]): Dictionary containing the fairness dataframes.
        model_name (str): The name of the model being evaluated (used for file naming).
        output_dir (Path): The directory where the resulting PNG charts will be saved.
    """
    output_dir.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    """
    Executes the comprehensive fairness analysis pipeline sequentially across the
    core machine learning models, generating CSV reports and visualization charts.
    """
